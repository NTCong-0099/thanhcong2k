...
if ( $nv_Request->get_int( 'save', 'post' ) == 1 )
{
    ...
    else
    {
        ...
        {
            //insert data
            $query = "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_rows` (`id`, `catid`, `title` ,`alias`, `hometext`, `bodytext`, `keywords`, `filepath`,`otherpath`, `roomid`,`fieldid`, `addtime`, `edittime`, `down`, `view`, `userid`, `status`, `type`,`sign`,`signtime`,`organid`)
	         		  VALUES (NULL, " . $db->dbescape( $data['catid'] ) . ", " . $db->dbescape( $data['title'] ) . ", " . $db->dbescape( $data['alias'] ) . ", " . $db->dbescape( $data['hometext'] ) . ",  " . $db->dbescape( $data['bodytext'] ) . ", " . $db->dbescape( $data['keywords'] ) . ", " . $db->dbescape( $data['filepath'] ) . ", " . $db->dbescape( $data['otherpath'] ) . ", " . $db->dbescape( $data['roomid'] ) . "," . $db->dbescape( $data['fieldid'] ) . ", " . $db->dbescape( $data['addtime'] ) . " ," . $db->dbescape( $data['edittime'] ) . "," . $db->dbescape( $data['down'] ) . "," . $db->dbescape( $data['view'] ) . "," . $db->dbescape( $data['userid'] ) . "," . $db->dbescape( $data['status'] ) . "," . $db->dbescape( $data['type'] ) . "," . $db->dbescape( $data['sign'] ) . "," . $db->dbescape( $data['signtime'] ) . "," . $db->dbescape( $data['organid'] ) . ")";
        ...
        elseif ( $id > 0 )
        {
            $query = "UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_rows` SET 
    				  ...
    				  `roomid`=" . $db->dbescape( $data['roomid'] ) . ", 
    				  `fieldid`=" . $db->dbescape( $data['fieldid'] ) . ",
    				  ...
    				  `edittime`=" . $db->dbescape( $data['edittime'] ) . ",
    				  `type` =" . $db->dbescape( $data['type'] ) . ",
    				  `sign` =" . $db->dbescape( $data['sign'] ) . ",
    				  `status` =" . $db->dbescape( $data['status'] ) . ",
    				  `signtime`=" . $db->dbescape( $data['signtime'] ) . ", 
    				  `organid`=" . $db->dbescape( $data['organid'] ) . " 
    				  WHERE id=" . intval( $id ) . "
    				  ";
            $db->sql_query( $query );
            nv_fix_catall_row ();
