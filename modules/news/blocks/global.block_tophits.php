...
if (!nv_function_exists('nv_news_block_tophits')) {
	function nv_block_config_tophits_blocks($module, $data_block, $lang_block) {$html = "";
		$html .= "<tr>";
	...
	function nv_news_block_tophits($block_config, $mod_data) {
		...
		while (list($id, $catid, $publtime, $exptime, $title, $alias, $homeimgthumb, $homeimgfile) = $db -> sql_fetchrow($result)) {
			if (!empty($homeimgfile)) {$array_img = explode("|", $homeimgfile);
		...
		if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/news/block_news.tpl")) {$block_theme = $module_info['template'];
